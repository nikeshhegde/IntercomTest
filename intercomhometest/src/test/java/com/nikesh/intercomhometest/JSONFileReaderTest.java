...
import com.nikesh.intercomhometest.utils.JsonFileReader;
...
public class JSONFileReaderTest {
	 ...
	 @Test
	    ...
	    public void getCustomersListFromJsonList() throws JSONException {
	        ...
	        List<Customer> customers = JsonFileReader.getCustomersListFromJsonList(customerJsonObjects);

	        assertEquals(3, customers.size());
	        assertEquals(7, customers.get(0).getUserId());
	        assertEquals("Eoin Gallagher", customers.get(1).getName());
	        assertEquals(0d, Double.compare(53.521111d, customers.get(2).getLatitude()) );
	        assertEquals(0d, Double.compare(-9.831111d, customers.get(2).getLongitude()) );
